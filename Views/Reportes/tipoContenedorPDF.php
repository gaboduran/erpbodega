...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(20,10,'LISTADO GENERAL TIPOS DE CONTENEDOR',0,0,'C');
...
$stringSQL = "SELECT ID, CODIGO, DESCRIPCION, ISOCODE, GRUPOCONT, CASE WHEN ESTADO = 1 THEN 'activo' ELSE 'inactivo' END AS ESTADO FROM tipocont";
...
try {
    ...
    $pdf->Cell(55,5,'DESCRIPCION',0,0,'L',1);
    $pdf->Cell(50,5,'GRUPO',0,0,'L',1);
    $pdf->Cell(30,5,'ISOCODE',0,0,'L',1);
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        foreach ($listado as $row => $column) {
            ...
            $pdf->Cell(55,5,ucwords($column["DESCRIPCION"]),0,0,'L',1);
            $pdf->Cell(52,5,ucwords($column["GRUPOCONT"]),0,0,'L',1);
            $pdf->Cell(42,5,ucwords($column["ISOCODE"]),0,0,'L',1);
    ...
    $pdf->Output('I','MaestrotipoContenedor');
