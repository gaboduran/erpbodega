...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(20,10,'LISTADO GENERAL TAMANO CONTENEDOR',0,0,'C');
...
try {
    ...
    $pdf->Cell(30,5,'TAMANO',0,0,'L',1);
    $pdf->Cell(60,5,'DESCRIPCION',0,0,'L',1);
    $pdf->Cell(65,5,'TEUS',0,0,'L',1);
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        foreach ($listado as $row => $column) {
            ...
            $pdf->setX(9);
            $pdf->Cell(30,5,$column["TAMANO"],0,0,'L',1);
            $pdf->Cell(62,5,ucwords($column["DESCRIPCION"]),0,0,'L',1);
            $pdf->Cell(75,5,ucwords($column["TEUS"]),0,0,'L',1);
    ...
    $pdf->Output('I','MaestroTamanoContenedor');
