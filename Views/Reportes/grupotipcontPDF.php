...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(20,10,'LISTADO GENERAL GRUPO TIPO DE CONTENEDOR ',0,0,'C');
...
$stringSQL = "SELECT ID, CODIGO, DESCRIPCION, CASE WHEN ESTADO = 1 THEN 'activo' ELSE 'inactivo' END AS ESTADO
FROM grupotipcont";
...
try {
    ...
    $pdf->Cell(100,5,'DESCRIPCION',0,0,'L',1);
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        foreach ($listado as $row => $column) {
            ...
            $pdf->setX(9);
            $pdf->Cell(50,5,$column["CODIGO"],0,0,'L',1);
            $pdf->Cell(120,5,ucwords($column["DESCRIPCION"]),0,0,'L',1);
    ...
    $pdf->Output('I','MaestroGrupoTipoContenedor');
