...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(20,10,'LISTADO GENERAL MONEDAS',0,0,'C');
...
$stringSQL = "SELECT ID, CODIGO, DESCRIPCION, CASE WHEN moneda.ESTADO = 1 THEN 'activo' ELSE 'inactivo' END AS ESTADO FROM moneda";
...
try {
    ...
    $pdf->Cell(110,5,'DESCRIPCION',0,0,'L',1);
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        foreach ($listado as $row => $column) {
            ...
            $pdf->Cell(122,5,ucwords($column["DESCRIPCION"]),0,0,'L',1);
