...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(20,10,'LISTADO GENERAL CLASIFICACION CONTENEDORES',0,0,'C');
...
$stringSQL = "SELECT clasificacion.ID, CODIGO, DESCRIPCION, NOMCLIENTE, CASE WHEN clasificacion.ESTADO = 1 THEN 'activo' ELSE 'inactivo' END AS ESTADO
                    FROM clasificacion 
                    INNER JOIN CLIENTE ON cliente.ID = clasificacion.IDLINEA";
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        foreach ($listado as $row => $column) {
            ...
            $pdf->Cell(90,5,ucwords($column["NOMCLIENTE"]),0,0,'L',1);
    ...
    $pdf->Output('I','MaestroClasificacion');
