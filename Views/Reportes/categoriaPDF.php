...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(98);
            // Title
            $this->Cell(20,10,'LISTADO GENERAL CATEGORIAS CONTENEDORES',0,0,'C');
...
$stringSQL = "SELECT categoria.ID, CODIGO, NOMCATEGORIA, cliente.NOMCLIENTE, CASE WHEN categoria.ESTADO = 1 THEN 'activo' ELSE 'inactivo' END AS ESTADO
                FROM categoria
                INNER JOIN CLIENTE ON cliente.ID = categoria.IDLINEA";
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        foreach ($listado as $row => $column) {
            ...
            $pdf->Cell(55,5,ucwords($column["NOMCATEGORIA"]),0,0,'L',1);
