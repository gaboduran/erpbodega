document.addEventListener("DOMContentLoaded",
	function () {
		tblCargo = $('#tblCargo').DataTable({
			...
			ajax: {
				url: `${base_url}Cargo/getAllCargo`,
			},
			columns: [
				{ data: "id" },
				{ data: "codigo" },
...
async function editaCargo(idcargo) {
	...
	validador.resetForm();
	try {
		const url = `${base_url}Cargo/editaCargo`;
		$.ajax({
			url: url,
			method: "POST",
...
async function procesar() {
	event.preventDefault();
	var valida;
	valida = $("#frmCargo").valid();
	if (valida == true) {
		let frmCargo = new FormData(document.querySelector("#frmCargo"));
		try {
			const url = `${base_url}Cargo/procesar`;
			...
			} else if (resultado.status == 'Cargo_existe') {
				Swal.fire({
					icon: 'error',
					title: 'Atención:',
					text: 'EL Codigo Cargo ya existe en el sistema'
				});
...
function deleteCargo(idCargo) {
	Swal.fire({
		...
		text: "Confirma que desea eliminar el Cargo seleccionada?",
		icon: 'warning',
		...
		if (result.isConfirmed) {
			...
			$('#ideliminacion').val(idCargo);
			$(document).on('submit', '#frmElimina', function (event) {
				event.preventDefault();
				$.ajax({
					url: `${base_url}Cargo/deleteCargo`,
					method: "POST",
...
function Print_Report(Criterio) {
	if (Criterio == 'Listado') {
		window.open("views/reportes/cargoPDF.php",
			'win2',
...
$(document).on('click', '#imprimir_excel', function (e) {
	window.location.href = `${base_url}views/reportes/cargoEXCEL.php`;
	e.preventDefault();
});
$(document).on('click', '.cargoModal', function () {
	$("#txt_codigo").removeClass("is-invalid");
	$("#txt_descripcion").removeClass("is-invalid");
	$("#sel_estado").removeClass("is-invalid");
	validador.resetForm();
...
==================================================================================================================*/
var validador = $('#frmCargo').validate({
	rules: {
		txt_codigo: {
			required: true,
		},
