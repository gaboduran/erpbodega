document.addEventListener("DOMContentLoaded",
	function () {
		tblReserva = $('#tblReserva').DataTable({
			order: [[0, 'desc']],
			select: true,
			"bProcessing": true,
			"serverSide": true,
			...
			ajax: {
				url: `${base_url}reserva/getAllReservas`,
			},
			columns: [
				{ data: "id" },
				{ data: "fechacrea" },
...
function carga_datatable(fechaini = '', fechafin = '', sel_linea = '', ti_salida = '', booking = '', idtransporte = '', tercero = '', sel_estado = '') {
	tblReserva = $('#tblReserva').DataTable({
		order: [[0, 'desc']],
		"bProcessing": true,
		"serverSide": true,
		...
		ajax: {
			method: "POST",
			url: `${base_url}reserva/getReportereserva`,
			data: { fechaini: fechaini, fechafin: fechafin, sel_linea: sel_linea, ti_salida: ti_salida, booking: booking, idtransporte: idtransporte, tercero: tercero, sel_estado: sel_estado }
		},
		columns: [
			{ data: "id" },
			{ data: "fechacrea" },
...
function filtrar() {
	...
	if (valida == true) {
		...
		var ti_salida = $('#sel_tisalidaindx').val();
		...
		if (fechaini != '' || fechafin != '' || sel_linea != '' || ti_salida != '' || booking != '' || idtransporte != '' || tercero != '' || sel_estado != '') {
			$('#tblReserva').DataTable().destroy();
			carga_datatable(fechaini, fechafin, sel_linea, ti_salida, booking, idtransporte, tercero, sel_estado);
		}
	}
}
